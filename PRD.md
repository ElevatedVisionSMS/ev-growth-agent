...
A "Submissions Report" link — visible only after entering the form in Internal (Elevated Vision Team) mode, not on the homepage and not in Client Self-Serve mode — leads to a login form (hardcoded demo credentials: username `elayne`, password `123abc`). Logging in shows a dashboard listing every report generated by any user of the tool, regardless of which mode they used — business name, industry, city, date, and the full intake + generated report as expandable detail. Every successful report generation is saved to shared persistent storage automatically.
